...
<div class="h-screen w-screen bg-cover" style="background-image: url('/assets/mountains.jpg');">
    <div class="absolute inset-0 bg-leaf-600 bg-opacity-95 flex flex-col gap-10 items-center font-fanwood">
        <div class="mt-12">
            {#if imageUrl}
                <img width=300 height=300 src={imageUrl} alt="Uploaded Profile" />
            {:else}
                <img width=200 height=200 src="assets/logos/logo_dark_bg.png" alt="Logo dark" />
            {/if}
        </div>
        <h1 class="text-lightwood-100 text-5xl text-center">Registration</h1>
